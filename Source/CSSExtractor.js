...
class ListofObj_to_Table {
  ...
  */
  static formatHTMLcellvalues(strCellinput) {
    ...
    let strOutput;
    if (ListofObj_to_Table.isValidHttpUrl(strCellinput)) { // if strCellinput is URL, then wrap A href tag around so it can be displayed as hyperlink
      strOutput =
        "<A HREF='" +
        strCellinput +
        "' target='_blank'>" +
  ...
  */
  static formatHTMLTableRows() {
    let strOutput = "<TR>";
    for (let i = 0; i < arguments.length; i++) {
        strOutput += "<TD>" + ListofObj_to_Table.formatHTMLcellvalues(arguments[i]) + "</TD>";
    }
    strOutput += "</TR>";
    return strOutput;
  }
  ...
  */
  createHTMLTable() {
    ...
    this.tableHeader = this.tableHeader.concat(Object.keys(this.tableObjSchema));
    ...
    this.strHTMLlines += (this.tableTitle.length > 0 ? "<H1>"+this.tableTitle+"</H1>" : "");
    this.strHTMLlines += ListofObj_to_Table.formatHTMLTableHeaders(...this.tableHeader);
    for (let i = 0; i < objCollection.length; i++) {
      let objItem = objCollection[i]; // get the object HTML collection item
      this.tableData.forEach((item)=>{this.collectionFields.push(eval(item))});
      this.strHTMLlines += ListofObj_to_Table.formatHTMLTableRows(i+1,...this.collectionFields);
      this.collectionFields = [];
    }
  ...
  */
  static formatCSVTableRows() {
    ...
    for (let i = 0; i < arguments.length; i++) {
      if (i < arguments.length-1) {
        strOutput += ListofObj_to_Table.escapeForCSV(ListofObj_to_Table.formatCSVcellvalues(arguments[i])) + ',';
      } else {
          strOutput += ListofObj_to_Table.escapeForCSV(ListofObj_to_Table.formatCSVcellvalues(arguments[i])) + '\n';
      }
    }
  ...
  */
  static createobjCSVTable(thisObj) {
    ...
    strCSVlines += ListofObj_to_Table.formatCSVTableHeaders(...thisObj.tableHeader);
    for (let i = 0; i < objCollection.length; i++) {
      let objItem = objCollection[i]; // get the object HTML collection item
      thisObj.tableData.forEach((item)=>{thisObj.collectionFields.push(eval(item))});
      strCSVlines += ListofObj_to_Table.formatCSVTableRows(i+1,...thisObj.collectionFields);
      thisObj.collectionFields = [];
    }
    return strCSVlines;
  }    
...
(function () {
  ...
  propDict = {
                ...
                "Page Name": document.title,
	  			"CSS Selector Used": cssSelectorwebscrape
            }
  let propPage = new PageProperty(propDict,pageTitle="WLA CSS Selector Web Scraper v02");
  strHTMLlines += propPage.displayPageHeaders();

  listoftestObject = document.querySelectorAll(cssSelectorwebscrape);
  tableschema = {"Extracted innerText":"objItem['innerText']", 
                 "Extracted outerHTML":"objItem['outerHTML']",
                 "CSS selector attributes":"objItem['attributes']" };

...
function formatHTMLcellvalues(strCellinput) {
  ...
  if (isValidHttpUrl(strCellinput)) {
    strOutput =
      "<A HREF='" + strCellinput + "' target='_blank'>" + strCellinput + "</A>";
  } else {
    strOutput =
...
strHTMLlines += formatHTMLTableHeaders("No", "Link URL", "Link Text","Link Protocol","Internal/External");
for (let i = 0; i < objCollection.length; i++) {
  ...
  strHTMLlines += formatHTMLTableRows(
	...
	checkInternalExternalLink(objItem["host"],pageHost)
  );
}
strHTMLlines += "</TABLE>";

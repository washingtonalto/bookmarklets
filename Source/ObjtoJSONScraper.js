...
*/
  ...
  */
      static setDivStyle(divBackgroundcolor, divPropertycolor) {
        let strOutput = "<STYLE>";
        strOutput +=
          ".propertyname { font-weight:bold; font-color:" +
          divPropertycolor +
          "; }" +
          ".outputarea { background-color: " +
          divBackgroundcolor +
          "; } ";
        strOutput += "</STYLE>";
        return strOutput;
      ...
      static recursiveObjformat(obj) {
        ...
        for (let property in obj) {
          if (obj.hasOwnProperty(property) && obj[property] == null) {
            ...
            strOutput +=
              "<LI><SPAN class='propertyname'>" + property + "</SPAN>: ";
            strOutput += Obj_to_JSON.recursiveObjformat(obj[property]);
            strOutput += "</LI>";
          } else if (
            obj.hasOwnProperty(property) &&
  ...
  */
      display(divBackgroundcolor = "lightblue", divPropertycolor = "blue") {
        ...
        let strOutput =
          "<STRONG>Ojbect selected</STRONG>: " + this.inputObjname;
        strOutput += Obj_to_JSON.setDivStyle(
          divBackgroundcolor,
          divPropertycolor
        );
  ...
  */
      static createJSONBloblink(objJSON, jsonFileName = "download.json") {
        ...
        let objJSONBlob = new Blob([objJSON], { type: "text/json" });
...
function validate_object
  Returns true if valid object else returns false
Parameter:
* objString - object String for validation
*/
    function validate_object(objString) {
      return typeof eval(objString) === "object";
    }
    ...
    let propPage = new PageProperty(
      propDict,
      (pageTitle = "WLA Object to JSON scraper v02")
    );
    let strHTMLlines = "";
    ...
    if (validate_object(objtoscrape)) {
      customObj = new Obj_to_JSON(objtoscrape);
      strHTMLlines += customObj.display();
    } else {
      strHTMLlines += "Invalid object selected: " + objtoscrape;
    }
